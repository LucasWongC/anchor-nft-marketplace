...
describe("ignore creators tests", () => {
  ...
  it("Prepare tests variables", async () => {
    ...
    const associatedTokanAccount = await Token.getAssociatedTokenAddress(
    ...
    const sellerAccountInfo = await provider.connection.getAccountInfo(
      sellerTokenAccount
    );
    if (sellerAccountInfo === null) {
      await marketplaceMint.createAssociatedTokenAccount(seller.publicKey);
    }
  ...
  it("sell order ignore creators", async function () {
    ...
    const key3 = new Uint8Array([
      222, 69, 143, 147, 205, 198, 36, 153, 236, 0, 28, 187, 64, 186, 228, 5,
      81, 132, 101, 163, 156, 82, 229, 90, 110, 114, 197, 43, 180, 12, 166, 60,
      129, 129, 90, 166, 171, 171, 157, 157, 181, 187, 241, 219, 237, 249, 53,
      211, 123, 133, 137, 91, 234, 31, 167, 39, 165, 59, 199, 203, 206, 80, 212,
      11,
    ]);
    let buyer = anchor.web3.Keypair.fromSecretKey(key3);

    let buyerTokenATA = await Token.getAssociatedTokenAddress(
      splToken.ASSOCIATED_TOKEN_PROGRAM_ID,
      TOKEN_PROGRAM_ID,
      marketplaceMint.publicKey,
      buyer.publicKey
    );
    const buyerTokenInfo = await provider.connection.getAccountInfo(
      buyerTokenATA
    );
    if (buyerTokenInfo === null) {
      const mm = await marketplaceMint.createAssociatedTokenAccount(
        buyer.publicKey
      );
