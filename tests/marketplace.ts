...
import { Marketplace } from "../js/types/marketplace";
...
describe("marketplace with mint", () => {
  ...
  it("Prepare tests variables", async () => {
    ...
    const key0 = new Uint8Array([
      91, 135, 79, 34, 176, 14, 98, 6, 168, 113, 140, 224, 149, 171, 54, 216,
      251, 55, 89, 127, 248, 162, 169, 104, 35, 141, 126, 122, 5, 213, 39, 126,
      106, 230, 5, 6, 210, 198, 19, 145, 13, 41, 254, 197, 253, 11, 1, 136, 18,
      246, 152, 197, 252, 46, 204, 230, 15, 207, 138, 169, 138, 15, 182, 170,
    ]);
    ...
    const key1 = new Uint8Array([
      218, 240, 112, 171, 149, 113, 51, 231, 110, 255, 95, 30, 191, 107, 242,
      54, 184, 45, 161, 197, 197, 85, 90, 152, 246, 1, 182, 32, 140, 81, 192,
      248, 134, 244, 250, 133, 142, 147, 155, 189, 241, 122, 152, 235, 22, 214,
      74, 17, 76, 177, 84, 87, 22, 76, 44, 63, 106, 227, 5, 216, 230, 33, 184,
      160,
    ]);
    ...
    const key2 = new Uint8Array([
      228, 96, 81, 192, 83, 233, 241, 76, 237, 254, 237, 7, 219, 66, 39, 54,
      109, 135, 238, 49, 9, 200, 77, 112, 206, 134, 246, 130, 67, 33, 80, 195,
      228, 5, 202, 155, 73, 215, 83, 214, 94, 226, 54, 245, 142, 46, 236, 9,
      198, 16, 4, 57, 14, 206, 79, 142, 64, 119, 197, 237, 131, 120, 231, 135,
    ]);
    ...
    // );
    let t_pubkey = new anchor.web3.PublicKey(
      "5MYGMMafYr7G3HymNbaFQnHCdPxgWqAZqHZQP1NiDEJa"
    );
  ...
  it("create marketplace", async () => {
    await program.methods
      ...
      .signers([admin])
      .rpc();
    let createdMarketplace = await program.account.marketplace.fetch(
      marketplacePDA
    );
    assert.equal(createdMarketplace.fees.toString(), fee.toString());
    assert.equal(
      createdMarketplace.mint.toString(),
      marketplaceMint.publicKey.toString()
    );
    assert.equal(
      createdMarketplace.authority.toString(),
      admin.publicKey.toString()
    );
    assert.equal(
      createdMarketplace.feesDestination.toString(),
      adminTokenAccount.address.toString()
    );
  });
  ...
  it("fail: create marketplace fee > 10000", async () => {
    ...
    await provider.connection.confirmTransaction(fromAirdropSignature);
    let tmpTokenAccount =
      await marketplaceMint.getOrCreateAssociatedAccountInfo(
        tmpAuthority.publicKey
      );

    let [failedMarketplacePDA, failedMarketplaceDump] =
  ...
  it("update marketplace fields", async () => {
    ...
    let updatedMarketplace = await program.account.marketplace.fetch(
      marketplacePDA
    );
    assert.equal(updatedMarketplace.fees.toString(), tmpFee.toString());
    assert.equal(
      updatedMarketplace.authority.toString(),
      tmpAuthority.publicKey.toString()
    );
    assert.equal(
      updatedMarketplace.feesDestination.toString(),
      tmpTokenAccount.address.toString()
    );

    //revert
    await program.methods
      .updateMarketplace(fee, adminTokenAccount.address, admin.publicKey)
      .accounts({
  ...
  it("update marketplace mint", async () => {
    ...
    let [newEscrowPDA, newEscrowDump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [
          Buffer.from("MARKETPLACE"),
          marketplacePDA.toBuffer(),
    ...
    let newAdminTokenAccount =
      await newMarketplaceMint.getOrCreateAssociatedAccountInfo(
        admin.publicKey
      );

    await program.methods
      .updateMarketplaceMint(
        newMarketplaceMint.publicKey,
        newAdminTokenAccount.address
      )
      .accounts({
        authority: admin.publicKey,
        marketplace: marketplacePDA,
    ...
    assert.equal(
      updatedMarketplace.feesDestination.toString(),
      newAdminTokenAccount.address.toString()
    );
    assert.equal(
      updatedMarketplace.mint.toString(),
      newMarketplaceMint.publicKey.toString()
    );
  ...
  it("create collection", async () => {
    await program.methods
      .createCollection(collectionName, creator.publicKey, collectionFee, false)
      .accounts({
        authority: admin.publicKey,
        marketplace: marketplacePDA,
    ...
    let createdCollection = await program.account.collection.fetch(
      collectionPDA
    );
    assert.equal(
      createdCollection.marketplaceKey.toString(),
      marketplacePDA.toString()
    );
    assert.equal(
      createdCollection.requiredVerifier.toString(),
  ...
  it("update collection", async () => {
    ...
    await program.methods
      .updateCollection(tmpFee, tmpName, tmpRequiredVerifier, false)
      .accounts({
        authority: admin.publicKey,
        marketplace: marketplacePDA,
        collection: collectionPDA,
    ...
    let updatedCollection = await program.account.collection.fetch(
      collectionPDA
    );
    assert.equal(
      updatedCollection.requiredVerifier.toString(),
      tmpRequiredVerifier.toString()
    );
    ...
    // reset
    await program.methods
      .updateCollection(collectionFee, collectionName, creator.publicKey, false)
      .accounts({
        authority: admin.publicKey,
        marketplace: marketplacePDA,
        collection: collectionPDA,
  ...
  it("create sell order", async () => {
    ...
    await program.methods
      .createSellOrder(price, quantity, sellerTokenAccount.address)
      .accounts({
    ...
    assert.equal(
      sellOrder.destination.toString(),
      sellerTokenAccount.address.toString()
    );
    let accountAfterSellOrderCreate = await nftMint.getAccountInfo(
  ...
  it("remove one item from sell order", async () => {
    ...
    let updatedAccount = await nftMint.getAccountInfo(
      sellerNftAssociatedTokenAccount
    );
  ...
  it("add one item to sell order", async () => {
    ...
    await program.methods
      .addQuantityToSellOrder(quantity)
      .accounts({
        authority: seller.publicKey,
        sellerNftTokenAccount: sellerNftAssociatedTokenAccount,
        vault: programNftVaultPDA,
  ...
  it("buy the nft", async () => {
    ...
    await provider.connection.confirmTransaction(fromAirdropSignature);
    let buyerNftAta = await nftMint.getOrCreateAssociatedAccountInfo(
      buyer.publicKey
    );
    let buyerMarketplaceAta =
      await marketplaceMint.getOrCreateAssociatedAccountInfo(buyer.publicKey);
    ...
    await program.methods
      ...
      .remainingAccounts([
        {
          pubkey: creatorTokenAccount.address,
          isWritable: true,
          isSigner: false,
        },
        { pubkey: sellOrderPDA, isWritable: true, isSigner: false },
        {
          pubkey: sellerTokenAccount.address,
          isWritable: true,
          isSigner: false,
        },
      ])
    ...
    let updatedAdminTokenAccount = await marketplaceMint.getAccountInfo(
      adminTokenAccount.address
    );
    assert.equal(updatedAdminTokenAccount.amount.toNumber(), 50);

    let updatedSellerTokenAccount = await marketplaceMint.getAccountInfo(
      sellerTokenAccount.address
    );
    assert.equal(updatedSellerTokenAccount.amount.toNumber(), 850);

    let updatedCreatorTokenAccount = await marketplaceMint.getAccountInfo(
      creatorTokenAccount.address
    );
    assert.equal(updatedCreatorTokenAccount.amount.toNumber(), 100);

    let buyerNftAtaAfterSell = await nftMint.getOrCreateAssociatedAccountInfo(
      buyer.publicKey
    );
    assert.equal(buyerNftAtaAfterSell.amount.toNumber(), 1);

    let updatedBuyerTokenAccount = await marketplaceMint.getAccountInfo(
      buyerMarketplaceAta.address
    );
