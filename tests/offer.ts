...
describe("marketplace with mint", () => {
  ...
  it("Prepare tests variables", async () => {
    ...
    [marketplacePDA, marketplaceDump] =
  ...
  it("remove nft offer", async () => {
    await program.methods
      .createBuyOffer(new anchor.BN(1000))
      .accounts({
    ...
    let updatedBuyerAccount = await marketplaceMint.getAccountInfo(
  ...
  it("create nft offer", async () => {
    ...
    assert.equal(
      buyOffer.destination.toString(),
      buyerNftTokenAccount.toString()
    );
  ...
  it("execute nft offer", async () => {
    ...
    let updatedBuyerNftAccount = await nftMint.getAccountInfo(
      buyerNftTokenAccount
    );
    assert.equal(updatedBuyerNftAccount.amount, 1);

    let updatedSellerAccount = await marketplaceMint.getAccountInfo(
      sellerTokenAccount.address
    );
    assert.equal(updatedSellerAccount.amount, 850);

    let updatedSellerNftAccount = await nftMint.getAccountInfo(
      sellerNftAssociatedTokenAccount
    );
    assert.equal(updatedSellerNftAccount.amount.toNumber(), 4);

    let updatedMarketplaceDestinationAccount =
      await marketplaceMint.getAccountInfo(adminTokenAccount.address);
    assert.equal(updatedMarketplaceDestinationAccount.amount.toNumber(), 50);

    let updatedCreator = await marketplaceMint.getAccountInfo(
