...
export class Marketplace {
  ...
  async createMarketplace(
    ...
    this.marketplacePDA = marketplacePDA;
    const mPDAAccount = await this.program.provider.connection.getAccountInfo(
      marketplacePDA
    );
    if (mPDAAccount != null) {
    ...
    const ePDAAccount = await this.program.provider.connection.getAccountInfo(
      escrowPDA
    );
    if (ePDAAccount != null) {
      console.log("Already created");
    ...
    return await this.program.methods
      .createMarketplace(mint, fees, feesDestination, owner.publicKey)
      .accounts({
  ...
  async createCollection(
    ...
    const cPDAAccount = await this.program.provider.connection.getAccountInfo(
      collectionPDA
    );
    if (cPDAAccount != null) {
